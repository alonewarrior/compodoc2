...
let appName = '@compodoc/ngd-transformer';
...
export class DotEngine {
    ...
    generateGraph(deps) {
        ...
        if (this.options.outputFormats.indexOf('dot') > -1 && this.options.outputFormats.indexOf(
            'svg') === -1 && this.options.outputFormats.indexOf('html') === -1) {
        ...
        if (this.options.outputFormats.indexOf('html') > -1) {
            generators.push(
                this.generateDot(template, deps).then(_ => this.generateSVG()).then(_ => this.generateHTML()));
    ...
    private generateSVG() {
        ...
        return viz.renderString(fs.readFileSync(this.paths.dot).toString(), {format: 'svg', engine: 'dot'})
            .then(vizSvg => {
                return new Promise((resolve, reject) => {
                    fs.outputFile(
                        this.paths.svg,
